...
local Tabs = {
    ...
    Sub = Window:AddTab({ Title = "Sub Tab", Icon = "badge-info"}),
    ...
    Other = Window:AddTab({ Title = "Other", Icon = "plus"}),
    SeaEvent = Window:AddTab({ Title = "Sea Event", Icon = "fish"}),
}
...
task.spawn(function()
    while task.wait() do
        if AutoPullLever then
            if not CheckItemBPCR("Mirror Fractal") or not CheckItemBPCR("Valkyrie Helm") then
                Notify("Not Enough Items", 10)
            elseif CheckItemBPCR("Mirror Fractal") and CheckItemBPCR("Valkyrie Helm") then
...
if Second_Sea or Third_Sea then 
    ...
    task.spawn(function()
        while task.wait() do
            if AutoFullyRaid and (Second_Sea or Third_Sea) and CheckIsRaiding() then
                pcall(function()
            ...
            elseif (Second_Sea or Third_Sea) and AutoFullyRaid then
                local buy = game.ReplicatedStorage.Remotes.CommF_:InvokeServer("RaidsNpc", "Select", SelectRaid) == 1
                Start_Raid()
            end
        end
    end)
...
--Function Other Tab--
--[[Ganteng Hub - Other Tab Functions]]--
    ...
    while task.wait() do
        if SailBoat then
            repeat wait() 
                ...
                if checkboat() and not check_roughsea() then
                    if (checkboat().VehicleSeat.Position - Vector3.new(-16207.501953125, 9.0863618850708, 475.1490783691406)).Magnitude > 50 then
                        if game:GetService("Players").LocalPlayer.Character.Humanoid.Sit == true then
                            TweenObject(CFrame.new(-32704.103515625, 12.557344436645508, 24089.923828125),checkboat().VehicleSeat, 300)
                            Nocliprock = true
                        end
                    end
                ...
                if check_roughsea() and checkboat() then
