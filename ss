...
local Q = require(game.ReplicatedStorage.Quests)
    ...
    function topos(Pos)
        if game.Players.LocalPlayer.Character.Humanoid.Health > 0 and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            ...
            tween = game:GetService("TweenService"):Create(game.Players.LocalPlayer.Character.PartTele, TweenInfo.new(Distance / 300, Enum.EasingStyle.Linear),{CFrame = Pos})
            tween:Play()
...
function Bring(nameMob,BringC,DistanceF,radius)
    ...
    for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
        if v.Name == inputed and DetectingPart(v) and v.Humanoid.Health > 0 and inputed2.Magnitude <= inputed3 then
            VelocityEnemys(v)
            TweenObject(inputed4, v.HumanoidRootPart, 350)
...
-- G2L["7"]["Size"] = UDim2.new(0, 528, 0, 2);
-- G2L["7"]["Position"] = UDim2.new(0.49818, 0, 0.18318, 0);
...
-- G2L["c"]["Size"] = UDim2.new(0, 528, 0, 2);
        ...
        if game.Players.LocalPlayer.PlayerGui.Main.Quest.Visible then
            if not MobLevel1OrMobLevel2() then
                for i, v in pairs(GetMobSpawnList(GetMob())) do
                    pcall(function()
                        if not MobLevel1OrMobLevel2() and getgenv().Kaitun == true then
...
function Auto_Kill_Shanda()
    while task.wait() do
        if game.Players.LocalPlayer.Data.Level.Value >= 14 and game.Players.LocalPlayer.Data.Level.Value < 120 and getgenv().Kaitun then
            if CheckEnemies("Shanda") then
                v = CheckEnemies("Shanda")
                pcall(function ()
                    repeat task.wait()
                        KillMonster(v.Name, true, getgenv().Kaitun == false)
                        set_content("Auto Farm: Killing Shanda")
...
task.spawn(function()
    while task.wait() do
        if getgenv().Kaitun then
            ...
            if level >= 1 and level < 14 or level > 310 and level <= 700 then
                pcall(function()
                    FarmLevel()
                end)
            elseif level >= 200 and level < 310 then 
                Auto_Saber()
            elseif level >= 14 and level < 80 then
                Auto_Kill_Shanda()
            elseif level >= 90 and level <= 200 then
                Auto_Hunter_Quest()
                wait(120)
                Hop()
            elseif level >= 700 and First_Sea then
                Auto_SecondSea()
            end
        else
            pcall(function()
