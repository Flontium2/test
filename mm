                    ...
                    for i, v in pairs(game.Workspace.Enemies:GetChildren()) do
                        if v.Name == MobLevel1OrMobLevel2() and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 and game.Players.LocalPlayer.PlayerGui.Main:FindFirstChild("Quest").Visible then
                            repeat task.wait()
                                ...
                                set_content("Auto Farm: Killing Mob " .. v.Name)
    ...
    while task.wait() do
        if game.Players.LocalPlayer.Data.Level.Value >= 14 and game.Players.LocalPlayer.Data.Level.Value < 120 and getgenv().Kaitun then
            if CheckEnemies("Shanda") then
                ...
                pcall(function ()
                    repeat task.wait()
                        KillMonster(v.Name, true, getgenv().Kaitun)
                        set_content("Auto Farm: Killing Shanda")
                    until not v or v.Humanoid.Health <= 0 or not v:FindFirstChild("Humanoid") or not v:FindFirstChild("HumanoidRootPart") or getgenv().Kaitun == false or game.Players.LocalPlayer.Data.Level.Value >= 120
                end)
            else
...
function Auto_Saber()
    while task.wait() do
        if CheckItemBPCR("Saber") then
            getgenv().Kaitun:Set(false)
    ...
    while task.wait() do
        if getgenv().Kaitun then
            ...
            elseif level >= 200 then 
                Auto_Saber()
            elseif level >= 14 and level < 120 then
                Auto_Kill_Shanda()
            end
        end
    end
...
task.spawn(function()
    while task.wait() do
        if getgenv().Kaitun or AutoFarmLevelUntil120 or AutoKillPlayerAfterLv120 then
...
for i = 1, 1 do
    ...
    if #bladehit > 0 then
        pcall(function()
            ...
            CmrFwLib.activeController.hitboxMagnitude = 250
